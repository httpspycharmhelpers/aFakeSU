...
/**
 * PRoot 库入口 (aFakeSU 嵌入用): 完整运行一个 proot 会话。
 *
 * 独立二进制形态: main() 薄转发到此函数 (cli.c 底部)。
 * 嵌入形态:      su 链接 proot 全部对象, 构造 proot 参数后直接调用本函数
 *                (-DPROOT_EMBEDDED 时 cli.c 不生成 main, 避免与 su 冲突)。
 */
int proot_main(int argc, char *const argv[])
{
	...
	if (status < 0) {
		/* aFakeSU: 嵌入 su 时连 execve 帮助文本一起静默。 */
		if (!proot_quiet)
...
#ifndef PROOT_EMBEDDED
int main(int argc, char *const argv[])
{
	return proot_main(argc, argv);
}
#endif /* !PROOT_EMBEDDED */
