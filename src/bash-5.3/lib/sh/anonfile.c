...
/* aFakeSU: bionic 只在 __ANDROID_API__ >= 30 声明 memfd_create;
 * MFD_NOEXEC_SEAL (0x8) 则到 API 35 才有, 这里手工补。
 * 编译时已用 -D__ANDROID_API__=30 打开声明, 低于 30 时回退到 syscall。 */
#if defined (HAVE_MEMFD_CREATE) && defined (__BIONIC__)
